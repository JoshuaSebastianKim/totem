...
module.exports = {
	init: function(initConfig) {
		...
		if (!initConfig.width)
			initConfig.width = 48;
		if (!initConfig.characterSet)
			initConfig.characterSet = "SLOVENIA";
		if (typeof(initConfig.removeSpecialCharacters) == "undefined") 
			initConfig.removeSpecialCharacters = false;
		if (typeof(initConfig.replaceSpecialCharacters) == "undefined")
			initConfig.replaceSpecialCharacters = true;
	...
	underline: function(enabled) {
		if (enabled)
			append(config.TXT_UNDERL_ON);
		else
			append(config.TXT_UNDERL_OFF);
		}
	,

	underlineThick: function(enabled) {
		if (enabled)
			append(config.TXT_UNDERL2_ON);
		else
			append(config.TXT_UNDERL_OFF);
		}
	,
	...
	upsideDown: function(enabled) {
		if (enabled)
			append(config.UPSIDE_DOWN_ON);
		else
			append(config.UPSIDE_DOWN_OFF);
		}
	,

	invert: function(enabled) {
		if (enabled)
			append(config.TXT_INVERT_ON);
		else
			append(config.TXT_INVERT_OFF);
		}
	,
	...
	drawLine: function() {
		...
		for (var i = 0; i < printerConfig.width; i++) {
			if (printerConfig.lineChar)
				append(new Buffer(printerConfig.lineChar));
			else
	...
	maxiCode: function(data, settings) {
		if (printerConfig.type == 'star') {
			...
			if (settings.mode) {
				if (settings.mode == 2)
					append(config.MAXI_MODE2)
				else if (settings.mode == 3)
					append(config.MAXI_MODE3)
				else if (settings.mode == 5)
					append(config.MAXI_MODE5)
				else if (settings.mode == 6)
					append(config.MAXI_MODE6)
				else
	...
	code128: function(data, settings) {
		if (printerConfig.type == 'star') {
			...
			if (settings) {
				if (settings.text == 1)
					append(config.BARCODE_CODE128_TEXT_1);
				else if (settings.text == 2)
					append(config.BARCODE_CODE128_TEXT_2);
				else if (settings.text == 3)
					append(config.BARCODE_CODE128_TEXT_3);
				else if (settings.text == 4)
					append(config.BARCODE_CODE128_TEXT_4);
				}
			else {
				append(config.BARCODE_CODE128_TEXT_2);
			}
			...
			if (settings) {
				if (settings.width == "SMALL")
					append(config.BARCODE_CODE128_WIDTH_SMALL);
				else if (settings.width == "MEDIUM")
					append(config.BARCODE_CODE128_WIDTH_MEDIUM);
	...
	_printImageBufferStar: function(width, height, data, callback) {
		...
		for (var i = 0; i < Math.ceil(height / 24); i++) {
			...
			append(new Buffer([
				0x1b,
				0x6b,
				parseInt(imageBuffer.length / 24),
				0x00
			]));
			append(imageBuffer);
			append(new Buffer("\n"));
	...
	pdf417: function(data, settings) {
		if (printerConfig.type == 'star') {
			...
			if (settings.truncated)
				append(config.PDF417_OPTION_TRUNCATED)
			else
				append(config.PDF417_OPTION_STANDARD)
...
var setInternationalCharacterSet = function(charSet) {
	if (printerConfig.type == 'star') {
		// ------------------------------ Star Character set ------------------------------
		if (charSet == "USA")
			return config.CHARCODE_PC437;
		if (charSet == "JAPANESE")
			return config.CHARCODE_JIS;
		if (charSet == "MULTI")
			return config.CHARCODE_PC850;
		if (charSet == "PORTUGUESE")
			return config.CHARCODE_PC860;
		if (charSet == "CANADIAN")
			return config.CHARCODE_PC863;
		if (charSet == "NORDIC")
			return config.CHARCODE_PC865;
		if (charSet == "WEU")
			return config.CHARCODE_WEU;
		if (charSet == "GREEK")
			return config.CHARCODE_GREEK;
		if (charSet == "HEBREW")
			return config.CHARCODE_HEBREW;
		if (charSet == "WESTEUROPE")
			return config.CHARCODE_PC1252;
		if (charSet == "CIRLILLIC")
			return config.CHARCODE_PC866;
		if (charSet == "LATIN2")
			return config.CHARCODE_PC852;
		if (charSet == "SLOVENIA")
			return config.CHARCODE_PC852;
		if (charSet == "EURO")
			return config.CHARCODE_PC858;
		if (charSet == "THAI42")
			return config.CHARCODE_THAI42;
		if (charSet == "THAI11")
			return config.CHARCODE_THAI11;
		if (charSet == "THAI13")
			return config.CHARCODE_THAI13;
		if (charSet == "THAI14")
			return config.CHARCODE_THAI14;
		if (charSet == "THAI16")
			return config.CHARCODE_THAI16;
		if (charSet == "THAI17")
			return config.CHARCODE_THAI17;
		if (charSet == "THAI18")
			return config.CHARCODE_THAI18;
		return null;
	} else {
		// ------------------------------ Epson Character set ------------------------------
		if (charSet == "USA")
			return config.CHARCODE_USA;
		if (charSet == "FRANCE")
			return config.CHARCODE_FRANCE;
		if (charSet == "GERMANY")
			return config.CHARCODE_GERMANY;
		if (charSet == "UK")
			return config.CHARCODE_UK;
		if (charSet == "DENMARK1")
			return config.CHARCODE_DENMARK1;
		if (charSet == "SWEDEN")
			return config.CHARCODE_SWEDEN;
		if (charSet == "ITALY")
			return config.CHARCODE_ITALY;
		if (charSet == "SPAIN1")
			return config.CHARCODE_SPAIN1;
		if (charSet == "JAPAN")
			return config.CHARCODE_JAPAN;
		if (charSet == "NORWAY")
			return config.CHARCODE_NORWAY;
		if (charSet == "DENMARK2")
			return config.CHARCODE_DENMARK2;
		if (charSet == "SPAIN2")
			return config.CHARCODE_SPAIN2;
		if (charSet == "LATINA")
			return config.CHARCODE_LATINA;
		if (charSet == "KOREA")
			return config.CHARCODE_KOREA;
		if (charSet == "SLOVENIA")
			return config.CHARCODE_SLOVENIA;
		if (charSet == "CHINA")
			return config.CHARCODE_CHINA;
		if (charSet == "VIETNAM")
			return config.CHARCODE_VIETNAM;
		if (charSet == "ARABIA")
			return config.CHARCODE_ARABIA;
		return null;
	}
};
...
var append = function(buff) {
	...
	if (!buffer && printerConfig.characterSet)
		buffer = setInternationalCharacterSet(printerConfig.characterSet);
